    ...
    children: [
      ...
      { kind: 'file', name: 'app.css', path: 'src/app.css' },
      {
        ...
        children: [
          ...
          { kind: 'file', name: 'tauri-types.ts', path: 'src/core/tauri-types.ts' },
            ...
            <div className="workspace-preset-menu__section" aria-label="workspace preset list">
              {workspacePresets.map((preset) => (
                <button
                  ...
                  aria-label={`${preset.name.toLowerCase()} ${preset.panels.length} panels · ${preset.emphasis.map((panel) => PANEL_META[panel].label).join(', ')}`}
                  onClick={() => {
        ...
        {tabs.map((tab) => {
          ...
          return (
            <div
              ...
              aria-label={`${meta.icon} ${meta.label}`}
              tabIndex={0}
              draggable
          ...
          {chatSessions.map((session) => {
            ...
            return (
              <div
                ...
                tabIndex={0}
                aria-selected={active}
                aria-label={`${AGENT_META[session.agent].icon} ${session.title}`}
          ...
          <div className="chat-stream">
            ...
            {toolCards.map((tool) => (
              <button
                className="tool-card"
                key={tool.id}
                aria-label={`${tool.name} ${tool.summary} ${tool.status}`}
            ...
            <div className="preview-launch-actions">
              ...
              <div className="preview-config__actions" aria-label="preview run mode">
                <button
                  ...
                  aria-label="run in window fills the preview pane"
                  onClick={runInWindow}
                >
                ...
                <button
                  ...
                  aria-label="run outside window opens externally"
                  onClick={runOutside}
                >
            ...
            {openFiles.map((file) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${meta.name}${meta.dirty ? ' modified' : ''} x`}
          ...
          <div className="quick-open" role="dialog" aria-label="quick open">
            ...
            <div className="quick-open__results">
              {filteredFiles.map((path) => {
                ...
                const resultLabel = [meta.name, path, meta.status, meta.diagnostics ? '!' : null].filter(Boolean).join(' ');
                return (
                  <button
                    key={path}
                    className={path === activeFile ? 'quick-open__result quick-open__result--active' : 'quick-open__result'}
...
function VerifySurface({ header }: { header: PanelHeaderProps }) {
  ...
  const removeCustomCheck = (id: string) => {
    setChecks((c) => c.filter((item) => !item.id.startsWith('c-custom-') || item.id !== id));
      ...
      <div className="verify-stack">
        <section className="verify-section">
          ...
          <div className="verify-section__footer">
            <button className="verify-add" aria-label="+ issue" onClick={() => setAddingProblem((v) => !v)}>create +</button>
          </div>
        </section>
        ...
        <section className="verify-section">
          ...
          <div className="checks-list">
            {checks.map((check) => {
              const isCustom = check.id.startsWith('c-custom-');
              return (
                <article
                  key={check.id}
                  draggable
          ...
          <div className="verify-section__footer">
            <button className="verify-add" aria-label="+ check" onClick={() => setAddingCheck((v) => !v)}>create +</button>
          </div>
        </section>
      </div>
          ...
          <div className="formation-board">
            ...
            {formationNodes.map((node) => (
              <button
                ...
                aria-label={`${node.role} ${node.status} ${node.detail} ${node.role} settings`}
